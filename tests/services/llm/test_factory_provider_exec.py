...
@pytest.mark.asyncio
async def test_complete_injects_hf_env_token_for_huggingface_router(monkeypatch) -> None:
    monkeypatch.setenv("HF_TOKEN", "hf_from_env_dummy_token")
    ...
    assert result == "ok"
    assert captured_config["config"].api_key == "hf_from_env_dummy_token"


@pytest.mark.asyncio
async def test_complete_injects_hf_env_token_for_openai_binding_without_openai_secret(
    monkeypatch,
) -> None:
    """Regression: Visualization path passes binding=openai + api.openai.com + api_key=''.

    HF_TOKEN must authenticate router.huggingface.co the same way as resolve_llm_runtime_config.
    """
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    monkeypatch.setenv("HF_TOKEN", "hf_fallback_when_openai_empty")
    provider = _FakeProvider()
    captured_config: dict[str, Any] = {}
    ...
    assert cfg.api_key == "hf_fallback_when_openai_empty"
    assert cfg.effective_url and "router.huggingface.co" in cfg.effective_url
