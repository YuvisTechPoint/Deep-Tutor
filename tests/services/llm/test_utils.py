...
def test_normalize_hf_openai_compat_base_url_rewrites_hub_site() -> None:
    """Hub website origins must map to the OpenAI-compatible router."""
    assert normalize_hf_openai_compat_base_url("https://huggingface.co") == HF_OPENAI_COMPAT_ROUTER_BASE
    assert normalize_hf_openai_compat_base_url("https://hf.co/foo/bar") == HF_OPENAI_COMPAT_ROUTER_BASE
    assert normalize_hf_openai_compat_base_url("") == HF_OPENAI_COMPAT_ROUTER_BASE


def test_normalize_hf_openai_compat_base_url_preserves_router() -> None:
    """Explicit router URLs should pass through unchanged."""
    url = "https://router.huggingface.co/v1"
    assert normalize_hf_openai_compat_base_url(url) == url


def test_sanitize_upstream_error_body_strips_html() -> None:
    """HTML error bodies should become a short hint, not raw markup."""
    html = '<html class="" lang="en"><meta name="description" content="test"'
    out = sanitize_upstream_error_body("text/html", html)
    assert "<html" not in out.lower()
    assert "router.huggingface.co" in out


def test_sanitize_upstream_error_body_truncates_plain_jsonish() -> None:
    """Non-JSON plaintext errors may be truncated."""
    long_msg = "x" * 400
    out = sanitize_upstream_error_body("application/json", long_msg)
    assert len(out) <= 285
    assert out.endswith("…")
