...
def test_public_output_filter_allows_hidream_chat_png(tmp_path: Path) -> None:
    service = PathService.get_instance()
    original_root = service._project_root
    original_user_dir = service._user_data_dir
    ...
    try:
        ...
        allowed.parent.mkdir(parents=True, exist_ok=True)
        allowed.write_text("png-bytes", encoding="utf-8")

        denied_py = allowed.with_suffix(".py")
        denied_py.write_text("# no", encoding="utf-8")

        assert service.is_public_output_path(
            "workspace/chat/chat/hidream/abc.png"
        ) is True
        assert service.is_public_output_path(
            "workspace/chat/chat/hidream/evil.py"
        ) is False
    finally:
        service._project_root = original_root
        service._user_data_dir = original_user_dir
