...
def test_llm_hf_token_env_reroutes_when_openai_key_missing(tmp_path: Path, monkeypatch):
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    monkeypatch.setenv("HF_TOKEN", "hf_integration_test_dummy_token")

    catalog = _build_catalog()
    resolved = resolve_llm_runtime_config(catalog=catalog, env_store=_empty_env(tmp_path))

    assert resolved.provider_name == "huggingface"
    assert resolved.api_key == "hf_integration_test_dummy_token"
    assert resolved.effective_url == "https://router.huggingface.co/v1"
    assert "/" in resolved.model
    assert "gpt-" not in resolved.model.lower()


def test_llm_hf_aliases_reroute_like_hf_token(tmp_path: Path, monkeypatch):
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    monkeypatch.delenv("HF_TOKEN", raising=False)
    monkeypatch.delenv("HUGGINGFACE_HUB_TOKEN", raising=False)
    monkeypatch.delenv("HUGGING_FACE_HUB_TOKEN", raising=False)
    monkeypatch.delenv("HF_API_TOKEN", raising=False)
    monkeypatch.setenv("HF_API_TOKEN", "hf_alias_dummy_token")

    catalog = _build_catalog()
    resolved = resolve_llm_runtime_config(catalog=catalog, env_store=_empty_env(tmp_path))

    assert resolved.provider_name == "huggingface"
    assert resolved.api_key == "hf_alias_dummy_token"


def test_llm_huggingface_hub_token_alias_reroutes(tmp_path: Path, monkeypatch):
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    monkeypatch.delenv("HF_TOKEN", raising=False)
    monkeypatch.delenv("HUGGINGFACE_HUB_TOKEN", raising=False)
    monkeypatch.setenv("HUGGINGFACE_HUB_TOKEN", "hf_hub_std_dummy_token")

    catalog = _build_catalog()
    resolved = resolve_llm_runtime_config(catalog=catalog, env_store=_empty_env(tmp_path))

    assert resolved.provider_name == "huggingface"
    assert resolved.api_key == "hf_hub_std_dummy_token"


def test_llm_hf_routes_when_openai_env_is_placeholder(tmp_path: Path, monkeypatch):
...
def test_llm_hf_routes_when_openai_key_is_no_key_placeholder(tmp_path: Path, monkeypatch):
    monkeypatch.setenv("OPENAI_API_KEY", "no-key")
    monkeypatch.setenv("HF_TOKEN", "hf_integration_test_dummy_token")
...
def test_llm_hf_model_id_defaults_to_huggingface_gateway(tmp_path: Path, monkeypatch):
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    monkeypatch.delenv("HF_TOKEN", raising=False)
    ...
    catalog = _build_catalog(
        llm_profile={
            ...
            "api_version": "",
            "extra_headers": {},
            "models": [{"id": "llm-m", "name": "Qwen3", "model": "Qwen/Qwen3-32B"}],
        }
    )
    resolved = resolve_llm_runtime_config(catalog=catalog, env_store=_empty_env(tmp_path))
...
def test_llm_openai_wins_when_openai_env_looks_valid(tmp_path: Path, monkeypatch):
    ...
    assert resolved.api_key.startswith("sk-")


def test_llm_hf_forced_via_llm_use_hf_router_env(tmp_path: Path, monkeypatch):
    monkeypatch.setenv("OPENAI_API_KEY", "sk-" + "x" * 48)
    monkeypatch.setenv("HF_TOKEN", "hf_integration_test_dummy_token")
    monkeypatch.setenv("LLM_USE_HF_ROUTER", "1")
