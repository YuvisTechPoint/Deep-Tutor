...
def test_status_redacts_model_for_non_admin(mu_isolated_root, as_user, monkeypatch):
    ...
    assert result["hidream_image"]["status"] == "not_configured"
    assert "endpoint" not in result["hidream_image"]
...
def test_status_hidream_endpoint_visible_only_to_admin(
    ...
    monkeypatch.setattr(system_router, "get_embedding_config", lambda: _FakeEmbedding())
    monkeypatch.setattr(system_router, "resolve_search_runtime_config", lambda: _FakeSearch())
    monkeypatch.setenv("HIDREAM_SERVICE_URL", "http://127.0.0.1:7860")

    try:
        with as_user("u_admin", role="admin"):
            admin_result = asyncio.run(system_router.get_system_status())
        assert admin_result["hidream_image"]["status"] == "configured"
        assert admin_result["hidream_image"]["endpoint"] == "http://127.0.0.1:7860"

        with as_user("u_alice", role="user"):
            user_result = asyncio.run(system_router.get_system_status())
        assert user_result["hidream_image"]["status"] == "configured"
        assert "endpoint" not in user_result["hidream_image"]
    finally:
        monkeypatch.delenv("HIDREAM_SERVICE_URL", raising=False)
