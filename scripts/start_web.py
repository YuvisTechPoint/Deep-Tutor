...
LLM / Hugging Face credentials are not handled here; configure them in ``.env``
at the project root. See ``.env.example`` and
``deeptutor.services.config.env_store.ENV_KEY_ORDER`` for canonical names
...
_EN_MESSAGES: dict[str, str] = {
    ...
    "marketing_site": "Marketing site: {url}",
...
MESSAGES: dict[str, dict[str, str]] = {
    ...
    "hi": {
        **_EN_MESSAGES,
        "backend": "बैकएंड",
        "frontend": "फ़्रंटएंड",
        "config_source": "कॉन्फ़िग स्रोत: {source}",
    },
}
...
def main() -> None:
    ...
    frontend_register = f"{frontend_root}/register"
    banner(
        "DeepTutor",
        [
            f"{_t(language, 'backend')}   http://localhost:{backend_port}",
            f"{_t(language, 'frontend')}  {frontend_register}",
        ],
    )
    log_info(dim(_t(language, "marketing_site", url=f"{frontend_root}/")))
    ...
    razorpay_pub = _root_env_value("NEXT_PUBLIC_RAZORPAY_KEY_ID") or _root_env_value(
        "RAZORPAY_KEY_ID"
    )
    env_local_body = (
    ...
    if razorpay_pub.strip():
        env_local_body += f"NEXT_PUBLIC_RAZORPAY_KEY_ID={razorpay_pub.strip()}\n"
    env_local_path.write_text(env_local_body, encoding="utf-8")
    ...
    try:
        ...
        log_success(
            _t(
                language,
                "open_url",
                url=bold(f"http://localhost:{frontend_port}/register"),
            )
        )
        print()
