...
def _save_ui_language(language: str, path: Path = INTERFACE_SETTINGS_PATH) -> None:
    ...
    if path.exists():
        try:
            ...
            pass
    payload["language"] = normalize_ui_language(language, "en")
...
def _choose_language() -> str:
    ...
    language = select(
        ...
        [
            ...
            ("hi", "हिन्दी", "Web UI in Hindi (wizard stays in English)"),
            ("bn", "বাংলা", "Web UI in Bengali — partial translation (wizard in English)"),
        ],
    )
