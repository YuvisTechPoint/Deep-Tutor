...
/** Copy full transcript as Markdown (e.g. share to email or docs). */
export async function copyChatMarkdownToClipboard(
  messages: MessageItem[],
  options: BuildChatMarkdownOptions = {},
): Promise<boolean> {
  if (!messages.length || typeof navigator === "undefined" || !navigator.clipboard)
    return false;
  const markdown = buildChatMarkdown(messages, options);
  try {
