...
import {
  appLanguageToBcp47Locale,
  normalizeLanguage,
  usesDenseScriptCategoryLabels,
...
function formatContextWindowUpdatedAt(
  ...
  return parsed.toLocaleString(appLanguageToBcp47Locale(language), {
    dateStyle: "medium",
    timeStyle: "short",
  });
}
...
function SettingsPageContent() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setLanguage(normalizeLanguage(settingsPayload.ui.language));
  ...
  // letter-spacing; dense scripts (e.g. Indic) read better without faux caps.
  const labelClass = (size: "sm" | "md" | "lg"): string => {
    if (usesDenseScriptCategoryLabels(language)) {
      ...
      <div className="mx-auto max-w-[960px] px-6 py-8">
        ...
        <div className="mb-5 flex flex-wrap items-center gap-x-8 gap-y-3">
          ...
          <div className="flex items-start gap-2">
            <span className="pt-0.5 text-[12px] text-[var(--muted-foreground)]">
              {t("Language")}
            </span>
            <div className="flex min-w-0 flex-col gap-1">
              <div className="flex flex-wrap gap-0.5 rounded-lg bg-[var(--muted)] p-0.5">
                {(["en", "hi", "bn"] as const).map((v) => (
                  <button
                    key={v}
                    type="button"
              ...
              {language === "bn" && (
                <span className="max-w-[min(100%,22rem)] text-[10px] leading-snug text-[var(--muted-foreground)]">
                  {t("language.bengali_partial")}
                </span>
              )}
            </div>
          </div>
                ...
                <div className="mt-0.5 truncate text-[11px] text-[var(--muted-foreground)]">
                  {(() => {
                    ...
                    return parsed.toLocaleTimeString(
                      appLanguageToBcp47Locale(language),
