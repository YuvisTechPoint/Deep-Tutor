...
function LoginPageContent() {
  ...
  useEffect(() => {
    ...
    if (registered) loginParams.registered = "1";
    const nextParam = searchParams.get("next");
    if (nextParam) loginParams.next = nextParam;

    const externalLogin = marketingLoginUrl(loginParams);
    if (externalLogin.startsWith("http://") || externalLogin.startsWith("https://")) {
    ...
    fetchAuthStatus().then((status) => {
      ...
      checkIsFirstUser().then((first) => {
        if (first) assignMarketingSignup();
      });
    });
  }, [router, next, registered, searchParams]);
  ...
  return (
    <div className="w-full max-w-sm">
      ...
      <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl shadow-sm px-8 py-8">
        ...
        <FirebaseGoogleButton nextPath={next} />
      </div>
