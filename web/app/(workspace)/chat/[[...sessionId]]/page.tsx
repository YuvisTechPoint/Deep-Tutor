...
interface CapabilityDef {
  value: string;
  label: string;
  /** Optional title for the page header (composer still uses ``label``). */
  headerLabel?: string;
  description: string;
  icon: LucideIcon;
...
export default function ChatPage() {
  ...
  /* Roadmap → AI Tutor hand-off context (passed via ?context=milestone:<id>) */
  const roadmapMilestoneId = useMemo(() => {
    const raw = searchParams?.get("context") ?? "";
    if (raw.startsWith("milestone:")) {
      return raw.slice("milestone:".length) || null;
    }
    return null;
  }, [searchParams]);

  const roadmapMilestoneLabel = useMemo(() => {
    if (!roadmapMilestoneId) return null;
    return roadmapMilestoneId
      .replace(/[_-]+/g, " ")
  ...
  const handleSend = useCallback(
    async (content: string) => {
      ...
      const memoryPayload = temporaryChat ? [] : [...memoryReferencesPayload];
      const historyForSend = temporaryChat ? [] : historyReferencesPayload;
      const skillsForSend = temporaryChat ? [] : skillsPayload;
      ...
      sendMessage(
        ...
        historyForSend,
        {
          ...
          ...(temporaryChat ? { persistUserMessage: false } : {}),
        },
  ...
  const handleCopyShareLink = useCallback(async () => {
    if (!state.sessionId) {
      showShareToast(t("Share link when ready") as string);
      setShareMenuOpen(false);
      return;
    }
    const origin =
      typeof window !== "undefined" ? window.location.origin : "";
    const url = `${origin}/chat/${state.sessionId}`;
    try {
      await navigator.clipboard.writeText(url);
      showShareToast(t("Link copied") as string);
    } catch {
      showShareToast(t("Copy failed") as string);
    }
    setShareMenuOpen(false);
  }, [state.sessionId, showShareToast, t]);

  const handleCopyTranscriptShare = useCallback(async () => {
    if (!state.messages.length) {
      showShareToast(t("Copy failed") as string);
      setShareMenuOpen(false);
      return;
    }
    const ok = await copyChatMarkdownToClipboard(state.messages, {
      title: getExportTitle(),
    });
    showShareToast(
      ok ? (t("Transcript copied") as string) : (t("Copy failed") as string),
    );
    setShareMenuOpen(false);
  }, [getExportTitle, showShareToast, state.messages, t]);
  ...
  return (
    ...
    >
      <div className="mx-auto w-full max-w-[960px] px-6 pt-3 pb-0">
        {roadmapMilestoneId && (
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2 rounded-xl border border-[#D4734B]/35 bg-[#D4734B]/10 px-3 py-2">
            <div className="flex items-center gap-2 text-[12px] text-[#f3b8a0]">
              <MapPin className="h-3.5 w-3.5" />
              <span className="font-semibold">
                {t("Continuing from roadmap")}:
              </span>
            ...
            <Link
              href="/roadmap"
              className="text-[11px] font-medium text-[#D4734B] underline-offset-2 hover:underline"
            >
              {t("Back to roadmap")}
            </Link>
          </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-y-2 gap-x-3">
          <span className="text-[15px] font-semibold tracking-[-0.01em] text-[var(--foreground)]">
            {t(activeCap.headerLabel ?? activeCap.label)}
          </span>
          <div
            ref={shareMenuRef}
          ...
          >
            ...
            <div className="relative">
              ...
              {shareMenuOpen && (
                <div
                  role="menu"
                  className="absolute right-0 top-[calc(100%+6px)] z-[80] w-[min(100vw-2rem,17rem)] rounded-xl border border-[var(--border)] bg-[var(--popover)] py-1 shadow-lg"
                >
                  <button
                    type="button"
                    role="menuitem"
                    onClick={() => void handleCopyShareLink()}
                    className="flex w-full flex-col gap-0.5 px-3 py-2 text-left text-[12px] text-[var(--foreground)] transition-colors hover:bg-[var(--muted)]/60"
                  >
                    <span className="font-medium">{t("Copy chat link")}</span>
                    <span className="text-[10px] font-normal leading-snug text-[var(--muted-foreground)]">
                      {t("Share link hint")}
                    </span>
                  </button>
                  <button
                    type="button"
                    role="menuitem"
                    onClick={() => void handleCopyTranscriptShare()}
                    disabled={!state.messages.length}
                    className="flex w-full flex-col gap-0.5 border-t border-[var(--border)]/40 px-3 py-2 text-left text-[12px] text-[var(--foreground)] transition-colors hover:bg-[var(--muted)]/60 disabled:cursor-not-allowed disabled:opacity-40"
                  >
                    <span className="font-medium">
                      {t("Copy transcript (Markdown)")}
                    </span>
                    <span className="text-[10px] font-normal leading-snug text-[var(--muted-foreground)]">
                      {t("Transcript share hint")}
                    </span>
                  </button>
                </div>
              )}
            ...
            <button
              ...
              className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-[12px] font-medium transition-colors ${
                temporaryChat
                  ...
                  : "border-[var(--border)]/50 text-[var(--muted-foreground)] hover:border-[var(--border)] hover:text-[var(--foreground)]"
              }`}
            >
              <MessageSquareDashed
              ...
              />
              {temporaryChat
                ? (t("Temporary chat on") as string)
                : (t("Temporary chat off") as string)}
            </button>
            <button
              type="button"
        ...
        {temporaryChat && (
          <p className="mt-1.5 max-w-2xl text-[11px] leading-relaxed text-[var(--muted-foreground)]">
            {t("Temporary chat hint")}
          </p>
        )}
        {shareFeedback && (
          <p
            className="mt-1.5 text-right text-[11px] font-medium text-[var(--primary)]"
            role="status"
            aria-live="polite"
          >
            {shareFeedback}
