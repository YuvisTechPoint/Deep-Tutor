...
/** Six-bar waveform glyph (voice slot in composer). */
function VoiceInputBarsIcon({ className }: { className?: string }) {
  const heights = [4, 8, 11, 6, 10, 4];
  return (
    <svg
      className={className}
      viewBox="0 0 20 14"
...
export default memo(function ChatComposer({
  ...
  const abortVoiceRecognition = useCallback(() => {
    try {
  ...
  const handleVoiceInputClick = useCallback(() => {
    ...
    rec.onresult = (event: SpeechRecognitionEvent) => {
      ...
      if (merged) inputHandleRef.current?.setValue(merged);
    };
    try {
      rec.start();
    } catch {
  ...
  }, [voiceListening, abortVoiceRecognition]);

  /** Empty text and no @-context: show voice control in the send slot. */
  const showVoiceInSendSlot = !hasContent && !hasReferences;
              ...
              <div className="ml-auto flex shrink-0 items-center gap-1.5">
                ...
                {isStreaming ? (
                  ...
                  <button
                    ...
                    className={`rounded-full border border-[var(--border)]/70 bg-[var(--secondary)] p-[7px] text-[var(--foreground)] shadow-sm transition-[background-color,opacity,box-shadow] hover:bg-[var(--muted)] hover:border-[var(--border)] ${voiceListening ? "animate-pulse" : ""}`}
                    aria-label={
                      voiceListening
