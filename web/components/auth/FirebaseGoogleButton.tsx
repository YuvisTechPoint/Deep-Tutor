...
export function FirebaseGoogleButton({
  ...
  if (!isFirebaseConfigured() || !AUTH_ENABLED) {
    return null;
  }
  ...
  async function handleClick() {
    ...
    try {
      ...
      if (!res.ok) {
        setErr(res.error ?? "Firebase sign-in failed");
        setBusy(false);
        return;
      }
      onDone?.();
  ...
  return (
    <div className="space-y-2">
      <button
        ...
        className="flex w-full items-center justify-center gap-2 rounded-lg border border-[var(--border)] bg-[var(--card)] px-4 py-2.5 text-sm font-medium text-[var(--foreground)] transition hover:bg-[var(--muted)] disabled:opacity-50"
      >
      ...
      {err ? (
        <p className="text-center text-xs text-red-500">{err}</p>
      ) : null}
    </div>
  );
}
