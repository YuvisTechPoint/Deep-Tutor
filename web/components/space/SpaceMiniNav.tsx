          ...
          return (
            <Link
              ...
              className={`group relative flex h-11 items-center gap-3 rounded-md px-3 transition-colors ${
                active
                  ? "bg-[#D4734B]/12 text-[#D4734B]"
                  : "text-[var(--muted-foreground)] hover:bg-[#D4734B]/8 hover:text-[var(--foreground)]"
              }`}
            >
              {active && (
                <span
                  aria-hidden
                  className="absolute left-0 top-1/2 h-5 w-[2.5px] -translate-y-1/2 rounded-r-full bg-[#D4734B]"
                />
              )}
              <Icon
                size={17}
                strokeWidth={active ? 2 : 1.7}
                className={`shrink-0 ${active ? "text-[#D4734B]" : ""}`}
              />
              <span
