...
class HiDreamImageTool(_PromptHintsMixin, BaseTool):
    """Proxy text-to-image generation via HiDream-O1-Image upstream Flask server."""

    def get_definition(self) -> ToolDefinition:
        return ToolDefinition(
            name="hidream_image",
            description=(
                "Generate an image from a detailed text prompt using HiDream-O1-Image. "
                "Requires a running upstream HiDream Flask server and HIDREAM_SERVICE_URL "
                "(CUDA). Saves PNG under the chat workspace and returns an /api/outputs URL path."
            ),
            parameters=[
                ToolParameter(
                    name="prompt",
                    type="string",
                    description="English scene description (composition, lighting, subjects, style).",
                ),
                ToolParameter(
                    name="width",
                    type="integer",
                    description="Output width in pixels (256–2048).",
                    required=False,
                    default=1024,
                ),
                ToolParameter(
                    name="height",
                    type="integer",
                    description="Output height in pixels (256–2048).",
                    required=False,
                    default=1024,
                ),
                ToolParameter(
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        if not prompt:
            raise ValueError("hidream_image requires a non-empty prompt")

        width = int(kwargs.get("width") if kwargs.get("width") is not None else 1024)
        height = int(kwargs.get("height") if kwargs.get("height") is not None else 1024)
        seed = int(kwargs.get("seed") if kwargs.get("seed") is not None else 32)
        ...
        url_path = f"/api/outputs/{rel}"
        return ToolResult(
            content=(
                f"HiDream-O1-Image generation finished.\n\n"
                f"Saved PNG path (relative to user data): {rel}\n"
                f"Same-origin URL: {url_path}"
            ),
            metadata={
                "relative_path": rel,
                "outputs_url_path": url_path,
                "provider": "hidream-o1-image",
            },
        )
