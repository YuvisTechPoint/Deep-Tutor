...
class BaseAgent(ABC):
    ...
    def __init__(
        ...
        self._apply_hf_hub_token_fallback_if_needed()
    ...
    def _apply_hf_hub_token_fallback_if_needed(self) -> None:
        """Fill HF Hub auth when the resolver left api_key empty (cached config / timing).

        Mirrors ``llm.factory._coerce_hf_token_into_config`` so agents created before
        env reload still pick up ``HF_TOKEN`` for ``router.huggingface.co``.
        """
        ...
        bind = (self.binding or "openai").strip().lower()
        if bind not in {"openai", "huggingface"}:
            return

        raw_base = (self.base_url or "").strip()
        if raw_base and is_local_llm_server(raw_base):
            return
        ...
        if not _should_use_hf_inference_router(
            active_api_key="",
        ...
        hf_override = (os.getenv("HF_INFERENCE_BASE_URL") or "").strip()
        normalized = normalize_hf_openai_compat_base_url(hf_override or raw_base or None)
        ...
        self.logger.debug(
            "HF_TOKEN fallback applied for %s (base=%s…)",
            bind,
            normalized[:40],
        )
