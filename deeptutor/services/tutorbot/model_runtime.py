...
def resolve_tutorbot_llm_config(bot_config: Any) -> LLMConfig:
    ...
    When neither is set, fall back to the surface-level ``HF_MODEL_TUTORBOT``
    override so all TutorBots can be pinned to a chosen open-source model
    (e.g. ``Qwen/Qwen2.5-Coder-32B-Instruct``) without per-bot configuration.
    """
    ...
    surface_override = get_model_router().model_for_feature("tutorbot")
    if surface_override:
        return base.model_copy(update={"model": surface_override})
    return base
