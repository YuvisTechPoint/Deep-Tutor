...
def _normalize_language(language: Any, default: str = "en") -> str:
    """Normalize persisted UI language to ``en`` | ``hi`` | ``bn``."""
    if language is None or language == "":
        language = default
    if isinstance(language, str) and language.strip():
        return normalize_ui_language(language, default if isinstance(default, str) else "en")
    if isinstance(default, str) and default.strip():
        return normalize_ui_language(default, "en")
    return "en"
