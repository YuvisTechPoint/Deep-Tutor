...
def _hf_default_chat_model() -> str:
    """Preferred chat model when using the Hugging Face OpenAI-compatible router.

    Router catalog changes frequently; override with ``HF_DEFAULT_LLM_MODEL``
    (full HF model id) for your account tier, latency, and task mix.
    """
...
def _looks_like_placeholder_openai_key(key: str) -> bool:
    """Treat common tutorial / invalid ``OPENAI_API_KEY`` values as unset.

    Real production secrets are longer; this avoids routing chat to
    ``api.openai.com`` with junk while a valid ``HF_TOKEN`` is present.
    """
    k = (key or "").strip()
    if not k:
        return True
    low = k.lower()
    if low in {"sk", "sk-", "changeme", "dummy", "placeholder", "test"}:
        return True
    placeholders = {
        "no-key",
        "sk-no-key-required",
    ...
    if "replace-me" in low or "paste-your" in low:
        return True
    if low.startswith("sk-test") or low.startswith("sk_demo"):
        return True
    # Typical mistake: truncated or fake ``sk-…`` secrets
    if low.startswith("sk-") and len(k) < 24:
        return True
    return False


def _should_rewrite_openai_base_to_hf_router(base_url: str) -> bool:
    u = (base_url or "").strip().lower()
    if not u:
        return True
    if "api.openai.com" in u:
        return True
    # Catalog/env sometimes uses the Hub website or short links instead of the router API.
    return normalize_hf_openai_compat_base_url(base_url).rstrip("/") != (base_url or "").strip().rstrip(
        "/"
    )


def _should_use_hf_inference_router(
    *,
    active_api_key: str,
    openai_env: str,
    force_hf_router: bool,
) -> bool:
    """Prefer HF OpenAI-compatible router when Hub token exists and OpenAI creds are absent/junk."""
    hf_hub = hf_hub_token_from_env()
    if not hf_hub.strip():
        return False
    ...
    cred_profile = (active_api_key or "").strip()
    cred_openai_env = (openai_env or "").strip()

    # Explicit non-placeholder OPENAI_API_KEY wins unless LLM_USE_HF_ROUTER is set above.
    if cred_openai_env and not _looks_like_placeholder_openai_key(cred_openai_env):
        return False

    if not cred_profile and not cred_openai_env:
        return True

    if not cred_profile and cred_openai_env and _looks_like_placeholder_openai_key(cred_openai_env):
        return True

    if (
        cred_profile
        and cred_openai_env
        and cred_profile == cred_openai_env
        and _looks_like_placeholder_openai_key(cred_openai_env)
    ):
        return True

    if cred_profile and _looks_like_placeholder_openai_key(cred_profile) and not cred_openai_env:
        return True
...
def _choose_resolved_provider(
    *,
    ...
    # Fully-qualified model IDs such as ``Qwen/Qwen3-32B`` and
    # ``deepseek-ai/DeepSeek-Coder-V2-Instruct`` are intended for a model
    # gateway. With no explicit provider, prefer the HF OpenAI-compatible router
    # instead of falling through to provider keyword matching or OpenAI with a
    # placeholder key.
    if "/" in model and not hint:
        hf = find_by_name("huggingface")
        if hf:
            return hf
    ...
    # OSS-first fallback: prefer a local OpenAI-compatible server over OpenAI.
    return find_by_name("vllm") or find_by_name("ollama") or find_by_name("openai") or PROVIDERS[0]
...
def resolve_llm_runtime_config(
    ...
    *,
    ...
    if not resolved_model:
        resolved_model = _hf_default_chat_model()
    ...
    hf_hub = hf_hub_token_from_env()
    openai_direct = _as_str(os.getenv("OPENAI_API_KEY"))
    force_hf_router = _truthy_env_flag(env_values.get("LLM_USE_HF_ROUTER")) or _truthy_env_flag(
        os.getenv("LLM_USE_HF_ROUTER")
    )
    if hf_hub.strip() and _should_use_hf_inference_router(
        active_api_key=active_api_key,
        openai_env=openai_direct,
        force_hf_router=force_hf_router,
    ):
        active_api_key = hf_hub
        if not active_api_base.strip() or _should_rewrite_openai_base_to_hf_router(active_api_base):
            active_api_base = normalize_hf_openai_compat_base_url(
                _as_str(os.getenv("HF_INFERENCE_BASE_URL"))
            )
        else:
            active_api_base = normalize_hf_openai_compat_base_url(active_api_base)
    ...
    inferred_model_provider = find_by_model(resolved_model)
    if spec.name == "huggingface":
        low = resolved_model.lower()
        looks_openai = inferred_model_provider and inferred_model_provider.name == "openai"
        looks_like_gpt_name = low.startswith("gpt-") or "gpt-4" in low or "gpt-3" in low
        if looks_openai or looks_like_gpt_name:
            resolved_model = _hf_default_chat_model()
...
def _resolve_embedding_provider(
    *,
    ...
    # OSS-first embedding fallback: prefer local vLLM/Ollama.
    return "vllm" if "vllm" in EMBEDDING_PROVIDERS else "ollama"
