...
def _extract_temporary_chat(config: dict[str, Any] | None) -> bool:
    """Ephemeral turn: no user/assistant DB rows; no memory refresh (client still streams)."""
    if not isinstance(config, dict):
        return False
    raw = config.pop("_temporary_chat", False)
    if isinstance(raw, bool):
        return raw
    if isinstance(raw, str):
...
class TurnRuntimeManager:
    ...
    async def _run_turn(self, execution: _TurnExecution) -> None:
        ...
        try:
            ...
            temporary_chat = _extract_temporary_chat(request_config)
            if temporary_chat:
