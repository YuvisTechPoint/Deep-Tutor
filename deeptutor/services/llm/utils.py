...
def sanitize_upstream_error_body(
    content_type: str | None,
    body: str,
    *,
    max_plaintext_len: int = 280,
) -> str:
    """Turn HTML / non-JSON upstream failures into short, actionable messages."""
    sample = (body or "").lstrip()
    lower_head = sample[:512].lower()
    looks_html = lower_head.startswith("<!doctype html") or lower_head.startswith("<html")
    ctype = (content_type or "").split(";")[0].strip().lower()

    if looks_html or ("html" in ctype and "application/json" not in ctype):
        return (
            "Received HTML instead of JSON — the inference base URL is wrong "
            "(for Hugging Face use https://router.huggingface.co/v1, not huggingface.co)."
        )

    stripped = (body or "").strip()
    if len(stripped) > max_plaintext_len:
        return stripped[:max_plaintext_len] + "…"
