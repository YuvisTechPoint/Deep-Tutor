...
def _coerce_hf_token_into_config(config: LLMConfig, provider_spec: Any) -> LLMConfig:
    """Fill HF Hub token into OpenAI-compatible configs when OpenAI keys are absent.

    Mirrors ``resolve_llm_runtime_config`` so call paths that still resolve to the
    **openai** provider spec (Visualization tests, explicit empty api_key, etc.)
    authenticate against ``router.huggingface.co`` when ``HF_TOKEN`` (or aliases)
    is set and no real ``OPENAI_API_KEY`` is present.
    """
    if not _looks_like_missing_api_key(config.api_key):
        return config
    token = hf_hub_token_from_env().strip()
    if not token:
        return config
    ...
    base_raw = (config.effective_url or config.base_url or "").strip()
    if base_raw and is_local_llm_server(base_raw):
        return config

    backend = getattr(provider_spec, "backend", "openai_compat") or "openai_compat"
    spec_name = getattr(provider_spec, "name", None)
    base_lower = base_raw.lower()

    gw = find_gateway(api_base=base_raw or None)
    looks_hf = (
        spec_name == "huggingface"
        or (gw is not None and gw.name == "huggingface")
        or ("router.huggingface" in base_lower)
    )

    openai_env = (os.getenv("OPENAI_API_KEY") or "").strip()
    force_hf = str(os.getenv("LLM_USE_HF_ROUTER") or "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
    }
    prefer_hf_router = _should_use_hf_inference_router(
        active_api_key=config.api_key or "",
        openai_env=openai_env,
        force_hf_router=force_hf,
    )

    hub_site_or_openai_host_mismatch = bool(base_raw) and (
        normalize_hf_openai_compat_base_url(base_raw).rstrip("/") != base_raw.rstrip("/")
    )
    openai_official_or_blank = not base_lower or "api.openai.com" in base_lower

    openai_compat_remote = backend == "openai_compat" and mode == "standard"

    # HF router host → always use Hub token when the OpenAI key slot is empty.
    if looks_hf:
        should_bind_hf_token = True
    elif (
        openai_compat_remote
        and prefer_hf_router
        and spec_name == "openai"
        and (openai_official_or_blank or hub_site_or_openai_host_mismatch or not base_raw)
    ):
        # Default OpenAI binding with no real OpenAI secret: mirror resolver behaviour.
        should_bind_hf_token = True
    else:
        should_bind_hf_token = False

    if not should_bind_hf_token:
        return config

    hf_inference_override = (os.getenv("HF_INFERENCE_BASE_URL") or "").strip()
    normalized = normalize_hf_openai_compat_base_url(
        hf_inference_override or base_raw or None,
    )
    # OpenAI's cloud endpoint does not accept HF Hub tokens — swap to the router.
    if "api.openai.com" in normalized.lower():
        normalized = HF_OPENAI_COMPAT_ROUTER_BASE
...
def _validate_remote_credentials(config: LLMConfig, provider_spec: Any) -> None:
    ...
    hf_here = bool(hf_hub_token_from_env().strip())
    hint = (
        "Set HF_TOKEN (or HF_API_TOKEN / HUGGINGFACE_HUB_TOKEN) in the **project root** `.env`, "
        "restart the backend, optionally `LLM_USE_HF_ROUTER=1`. "
        "Or configure a provider in Settings, or point LLM_HOST at local Ollama / LM Studio / vLLM."
    )
    if not hf_here:
        hint += " Right now **no** Hugging Face token is visible to this Python process."
...
async def complete(
    ...
    **kwargs: Any,
    ...
    config = _coerce_hf_token_into_config(config, provider_spec)
